...
Creates 3-panel figures for each time frame:
...
-----
  ...
  python tests/plot_label_grow_comparison.py \\
      --bfs_dir ~/data/demo/benchmark_results/cloudid_bfs/ \\
      --edt_dir ~/data/demo/benchmark_results/cloudid_edt/ \\
      --tb_dir ~/data/demo/mcs_tbpf/idealized/test4/input/ \\
      --outdir ~/data/demo/benchmark_results/figures/
...
Requirements
------------
  matplotlib, numpy, xarray (available in pyflextrkr environment)
...
def parse_args():
    ...
    parser.add_argument(
        "--tb_dir", default="",
        help="Directory with input Tb .nc files (for background plotting)",
    )
...
def load_tb_for_frame(tb_dir, frame_name):
    """Try to load corresponding Tb data for a given frame."""
    if not tb_dir:
        return None
...
def plot_comparison(bfs_data, edt_data, tb, frame_name, outdir):
    """Create 3-panel comparison figure for one frame."""
    ...
    if n_diff > 0:
        # Red where BFS has label but EDT doesn't (or different)
    ...
    outfile = os.path.join(outdir, f"comparison_{frame_name.replace('.npz', '.png')}")
...
def main():
    ...
    for frame_name in common:
        ...
        # Try to load Tb for background
        tb = load_tb_for_frame(args.tb_dir, frame_name)

        outfile = plot_comparison(bfs_data, edt_data, tb, frame_name, args.outdir)
